			...
			for(int i=0; i<7; i++){
				if(dayofweekIC[i].contains(" ")){//공백은 구분자로서 이미지 유무를 판단함
					...
					while(j<dayofweekICnum[i]){//이미지 토큰 갯수만큼 반복해줌.
						...
						System.out.println("  ****"+i+","+j+"에 "+Main.route+CFC.serialCode+"\\"+dayofweekImagename[i][j]+".png 로드. 넓이/크기"+dayofweekImage[i][j].getIconWidth()+"/"+dayofweekImage[i][j].getIconHeight());
						j++;
					}
				}
				else{//내용없을경우 아무것도 안함
