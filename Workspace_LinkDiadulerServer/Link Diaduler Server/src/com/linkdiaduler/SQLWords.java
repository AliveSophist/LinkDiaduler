...
public class SQLWords {
	...
	public static void createTempTable(){//계쩡테이블 생성후 즉시 이행
		...
		try{
			...
			System.out.println("temp(forClone) table is creating... wait for few minutes...");
	...
	public static boolean returnDatabaseExists(String dbName){
		Connection conn = null;
		try{
			...
			while(rs.next()){
				...
				if(dbName.equals(catalogs)){
					System.out.println(dbName+" DB exists.\n\n");
					return true;
				}
			}
			rs.close();
		...
		System.out.println(dbName+" DB does not exists. Let's make DB!\n\n");
		return false;
	}
